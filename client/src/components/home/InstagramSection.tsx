...
import { mockInstagramPosts } from '@/lib/mockData';
...
export default function InstagramSection() {
  const [posts, setPosts] = useState<InstaPost[]>(mockInstagramPosts);
    ...
    <section className="section-padding">
      <div className="container-luxury">
        <SectionHeading
          ...
          description="Behind the scenes of our latest installations and showroom highlights."
        />
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {posts.map((post, i) => (
            <motion.a
              key={post.id}
              href={post.permalink || BRAND.social.instagram}
              target="_blank"
            ...
            >
              ...
              <div className="absolute inset-0 bg-obsidian/0 group-hover:bg-obsidian/60 transition-colors duration-300 flex items-center justify-center">
                <svg
                  className="w-6 h-6 text-cream opacity-0 group-hover:opacity-100 transition-opacity"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
