...
router.post('/', authenticate, async (req, res) => {
    ...
    const id = await insert('INSERT INTO categories (name, slug, description, image, parent_id) VALUES (?, ?, ?, ?, ?)', [name, slug, description || null, image || null, parent_id ? Number(parent_id) : null]);
...
router.put('/:id', authenticate, async (req, res) => {
    ...
    await query('UPDATE categories SET name=?, slug=?, description=?, image=?, parent_id=? WHERE id=?', [
        name ?? existing.name,
        slug,
