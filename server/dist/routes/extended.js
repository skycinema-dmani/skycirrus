...
import { mapProject, mapTestimonial, mapService, mapBlogPost, mapAppointment } from '../utils/mappers.js';
const router = Router();
...
router.get('/projects', async (req, res) => {
    ...
    const rows = await query(`SELECT * FROM projects ${where} ORDER BY featured DESC, created_at DESC${limitClause}`, params);
...
router.get('/projects/:slug', async (req, res) => {
    const row = await queryOne('SELECT * FROM projects WHERE slug = ?', [req.params.slug]);
    if (!row)
    ...
    sendSuccess(res, mapProject(row, images));
});
router.post('/projects', authenticate, upload.single('cover_image'), async (req, res) => {
    ...
    if (!cover_image)
        return sendError(res, 'Project cover image is required');
    const id = await insert('INSERT INTO projects (title, slug, location, type, description, equipment, testimonial, client_name, featured, cover_image, before_after) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [
        title,
        slug,
        location,
        type,
...
router.put('/projects/:id', authenticate, upload.single('cover_image'), async (req, res) => {
    ...
    await query('UPDATE projects SET title=?, slug=?, location=?, type=?, description=?, equipment=?, testimonial=?, client_name=?, featured=?, cover_image=?, before_after=? WHERE id=?', [
...
router.get('/testimonials', async (req, res) => {
    ...
    const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
    const rows = await query(`SELECT * FROM testimonials ${where} ORDER BY featured DESC, created_at DESC`, params);
...
router.post('/testimonials', authenticate, upload.single('client_photo'), async (req, res) => {
    ...
    const id = await insert('INSERT INTO testimonials (client_name, client_photo, rating, review, location, featured) VALUES (?, ?, ?, ?, ?, ?)', [client_name, client_photo || null, Number(rating) || 5, review, location || null, featured === 'true' || featured === true || featured === '1' ? 1 : 0]);
    const created = await queryOne('SELECT * FROM testimonials WHERE id = ?', [id]);
...
router.put('/testimonials/:id', authenticate, upload.single('client_photo'), async (req, res) => {
    const id = Number(req.params.id);
    const existing = await queryOne('SELECT * FROM testimonials WHERE id = ?', [id]);
    if (!existing)
    ...
    await query('UPDATE testimonials SET client_name=?, client_photo=?, rating=?, review=?, location=?, featured=? WHERE id=?', [client_name ?? existing.client_name, client_photo, rating ? Number(rating) : existing.rating, review ?? existing.review, location ?? existing.location, featured !== undefined ? (featured === 'true' || featured === true || featured === '1' ? 1 : 0) : existing.featured, id]);
    const updated = await queryOne('SELECT * FROM testimonials WHERE id = ?', [id]);
...
router.get('/services', async (_req, res) => {
    const rows = await query('SELECT * FROM services ORDER BY sort_order ASC, name ASC');
...
router.get('/blog', async (req, res) => {
    ...
    const limitClause = limit ? ` LIMIT ${Math.min(Number(limit) || 10, 100)}` : '';
    const rows = await query(`SELECT * FROM blog_posts ORDER BY published_at DESC${limitClause}`);
    sendSuccess(res, rows.map(mapBlogPost));
});
router.get('/blog/:slug', async (req, res) => {
    const row = await queryOne('SELECT * FROM blog_posts WHERE slug = ?', [req.params.slug]);
    if (!row)
...
router.post('/blog', authenticate, async (req, res) => {
    ...
    if (!title || !excerpt || !content || !published_at)
        return sendError(res, 'Missing required fields');
    const slug = slugify(req.body.slug || title);
    ...
    const created = await queryOne('SELECT * FROM blog_posts WHERE id = ?', [id]);
    sendSuccess(res, created ? mapBlogPost(created) : null, 'Blog post created', 201);
});
router.put('/blog/:id', authenticate, async (req, res) => {
    const id = Number(req.params.id);
    const existing = await queryOne('SELECT * FROM blog_posts WHERE id = ?', [id]);
    if (!existing)
        return sendError(res, 'Blog post not found', 404);
    const { title, excerpt, content, cover_image, author, published_at, meta_title, meta_description } = req.body;
    const slug = req.body.slug ? slugify(req.body.slug) : existing.slug;
    ...
    const updated = await queryOne('SELECT * FROM blog_posts WHERE id = ?', [id]);
...
router.post('/appointments', async (req, res) => {
    ...
    if (!name || !phone || !email || !city || !project_type || !budget || !preferred_date)
        return sendError(res, 'Missing required fields');
    ...
    const created = await queryOne('SELECT * FROM appointments WHERE id = ?', [id]);
...
router.get('/admin/dashboard', authenticate, async (_req, res) => {
    ...
    const [blogPosts] = await query('SELECT COUNT(*) AS count FROM blog_posts');
    sendSuccess(res, {
        users: users?.count || 0,
...
router.put('/admin/settings', authenticate, async (req, res) => {
    ...
    if (!updates || typeof updates !== 'object')
        return sendError(res, 'Invalid settings');
