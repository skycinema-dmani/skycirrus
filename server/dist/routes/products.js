...
router.get('/', async (req, res) => {
    ...
    const rows = await query(`SELECT p.*, c.name AS category_name
     FROM products p
     JOIN categories c ON c.id = p.category_id
     WHERE ${sql}
     ORDER BY p.featured DESC, p.name ASC${limitClause}`, params);
    sendSuccess(res, rows.map(mapProduct));
});
router.get('/:slug', async (req, res) => {
    const row = await queryOne(`SELECT p.*, c.name AS category_name
     FROM products p
     JOIN categories c ON c.id = p.category_id
...
router.post('/', authenticate, upload.single('image'), async (req, res) => {
    ...
    if (!image)
        return sendError(res, 'Product image is required');
    const newId = await insert(`INSERT INTO products (name, slug, model, brand, category_id, description, specifications, image, featured, series)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
        name,
        slug,
        model,
    ...
    const created = await queryOne(`SELECT p.*, c.name AS category_name FROM products p JOIN categories c ON c.id = p.category_id WHERE p.id = ?`, [newId]);
...
router.put('/:id', authenticate, upload.single('image'), async (req, res) => {
    ...
    await query(`UPDATE products SET name=?, slug=?, model=?, brand=?, category_id=?, description=?, specifications=?, image=?, featured=?, series=? WHERE id=?`, [
        name ?? existing.name,
        slug,
    ...
    const updated = await queryOne(`SELECT p.*, c.name AS category_name FROM products p JOIN categories c ON c.id = p.category_id WHERE p.id = ?`, [id]);
